...
- 横条钉在桌面（置顶、可拖拽、位置记忆），悬停 200ms 展开详情卡片，移出 300ms 收起；
- 托盘菜单：立即刷新 / 置顶开关 / 退出；左键托盘图标聚焦小组件；
- 数据每 60s 自动刷新，失败退避 1m→2m→5m，401 自动重读凭证/刷新 token。
...
- 沙箱同样会拦截对凭证文件的写回（应用会记日志并继续，读侧不受影响），普通环境不受限。
...
src-tauri/          Rust 后端（tauri 2 + reqwest + tokio）
  ...
  src/poller.rs     60s 轮询、退避、401 重试、事件推送
ui/                 原生 HTML/CSS/JS 前端（无框架）
preview/index.html  界面样式基准
docs/DESIGN.md      设计文档 + 接口探测记录（附录 A）
```
